  ...
  return (
    <section className="section-y overflow-hidden bg-[linear-gradient(150deg,var(--color-navy-950)_0%,var(--color-navy-850)_55%,var(--color-navy-800)_100%)] text-white">
      <div className="container-px grid items-center gap-14 lg:grid-cols-[1.05fr_0.95fr]">
        <Reveal>
          <h2 className="mb-5 text-3xl md:text-4xl lg:text-5xl font-bold leading-tight text-white">Complete Fluid Management Ecosystem</h2>
          <p className="mb-8 max-w-[460px] text-base md:text-lg leading-7 text-slate-300">Ponshankar Agencies provides end-to-end piping systems combining premium pipes, fittings, valves, accessories, and technical support. We don't just sell products; we engineer ecosystems.</p>
          ...
          <div className="grid max-w-[440px] grid-cols-2 gap-x-6 gap-y-6">
            {STATS.map((s) => (
              <div key={s.label} className="border-l-[3px] border-brand-500 pl-4">
                <b className="block font-display text-4xl font-extrabold leading-none text-white">{s.value}</b>
                <span className="mt-2 block text-sm font-medium tracking-widest text-slate-400">{s.label}</span>
              </div>
            ))}
          </div>
        </Reveal>
        <Reveal delay={0.1}>
          <div className="relative aspect-[4/3]">
            ...
            <div className="absolute -right-1.5 bottom-[26px] max-w-[210px] rounded-xl bg-white p-[16px_18px] text-ink shadow-[var(--shadow-lift)]">
              <b className="mb-2 block text-base font-semibold">Technical Support Excellence</b>
